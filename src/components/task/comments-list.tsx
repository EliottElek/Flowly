...
const CommentsList = ({ comments }: { comments: NestedComment[] }) => {
    ...
    const Comment = ({ comment }: { comment: NestedComment }) => {
        ...
        return (
            <li>
                <div>
                    <div className='flex p-2 flex-col bg-muted/50 border shadow-sm rounded-md gap-2'>
                        ...
                        <div className='m-1 opacity-80 !text-xs'>
                            <CommentContent content={comment.content} />
                        </div>
                        <div>
                            <Button onClick={() => setReply(!reply)} className='h-6 px-1 text-xs opacity-50' variant={"ghost"}>{reply ? "Cancel" : "Reply"}</Button>
                        </div>
                        {reply &&
                            <div className='border rounded-md p-2 bg-background'>
                                <Editor content={content} setContent={setContent} />
                                <form className='flex justify-end'>
                                    <Button disabled={JSON.stringify(content) === JSON.stringify({})} className='font-bold'>Reply</Button>
                                </form>
                            </div>
                        }
                    </div>
